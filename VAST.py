...
WRITE_MODEL = 'gpt2'
CHART_MODEL = 'GPT-2'
SETTINGS = ('random', 'bleached', 'aligned', 'misaligned', 'true_random')
SETTING_ONE =  'bleached'
...
def get_dicts_and_lists(all_terms_list, weat_terms_orig):
    ...
    if SETTING == 'misaligned':
        with open(path.join(p_cwe_dictionaries, f'{WRITE_MODEL}_aligned.pkl'), 'rb') as pkl_reader:  # explained in report
            weat_dict = pickle.load(pkl_reader)
    else:

        weat_dict = {key: embedding_dict[key] for key in weat_terms_orig
                     if key in term_list}
    weat_terms = list(weat_dict.keys())
    return embedding_dict, weat_dict, term_list, weat_terms


def vast_by_lexicon(target_w):
    lexicon_valence = []
    lexicon_dominance = []
    lexicon_arousal = []
    for idx, lexicon_target in enumerate(target_w):
        ...
        if len(SETTINGS) > 1:
            label_name = f'{WRITE_SETTING}'
            plt.plot(LAYERS, layerwise_valence, label = label_name , marker = 'o')
        else:
            label_name = f'{LEXICON[idx]} '

            plt.plot(LAYERS, layerwise_valence, label = label_name+ 'Valence', marker = 'o')
            if idx > 0:
                lexicon_dominance.append(layerwise_dominance)
                plt.plot(LAYERS, layerwise_dominance, label=label_name+ 'Dominance', marker = 'o')
                lexicon_arousal.append(layerwise_arousal)
                plt.plot(LAYERS, layerwise_arousal, label=label_name+ 'Arousal', marker = 'o')

        return plt


def vast_by_lexicon_plot(title):
    plt.xlabel('Layer')
...
def tokenization_analysis(weat_dict_i):
    ...
    if POLAR_TOKENIZATION == 'multi':  # multi_pleasant needs to be embedded
        ...
        if SETTING =='misaligned':
            with open(path.join(p_cwe_dictionaries, f'{WRITE_MODEL}_aligned.pkl'), 'rb') as pkl_reader:
                weat_dict_i = pickle.load(pkl_reader)
...
def pc_removal_analysis():
    global warriner_terms_valence, warriner_valence, warriner_terms_dominance, warriner_dominanc
    global warriner_terms_arousal, warriner_arousal
...
if __name__ == "__main__":
    global embedding_dict, weat_dict, term_list, weat_terms, SETTING, WRITE_SETTING
    ...
    if DO_VAST_SETTING:
        for s in SETTINGS:
            SETTING = s
            WRITE_SETTING = SETTING.capitalize()
            embedding_dict, weat_dict, term_list, weat_terms = get_dicts_and_lists(all_terms_list, weat_terms_orig)
            target_w = (bellezza_terms, ) # (bellezza_terms, anew_terms)
            vast_by_lexicon(target_w)
        if len(SETTINGS) > 0:
            title = f'{CHART_MODEL} {LEXICON[0]} VAST Score by Setting'
        else:
            title = f'{CHART_MODEL} Valence, Arousal, and Dominance by Lexicon - {WRITE_SETTING} Setting'
        vast_by_lexicon_plot(title)
